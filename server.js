...
// ============================================================
//  TOÀN BỘ THUẬT TOÁN DỰ ĐOÁN (từ file ttoan.txt)
// ============================================================
...
class UltraDicePredictionSystem {
    ...
    // --- CÁC MODEL (giữ nguyên từ ttoan.txt) ---
...
app.get('/predict', async (req, res) => {
    try {
        ...
        // Nếu API trả về object có trường data, lấy data
        if (sessions && sessions.data && Array.isArray(sessions.data)) {
            sessions = sessions.data;
        }
        // Nếu sessions không phải mảng, thử lấy từ trường khác
        if (!Array.isArray(sessions)) {
            // Có thể trả về mảng trực tiếp hoặc trong 'list', 'results', ...
            if (sessions && sessions.list && Array.isArray(sessions.list)) {
                ...
                // Nếu không có gì, báo lỗi
                return res.status(400).json({ error: 'Không thể phân tích dữ liệu từ API' });
            }
        }

        if (sessions.length === 0) {
            return res.status(400).json({ error: 'Không có dữ liệu phiên' });
        }

        // Lọc ra các phiên có kết quả hợp lệ (T/X)
        const validSessions = sessions.filter(s => s.result && (s.result === 'T' || s.result === 'X'));
        if (validSessions.length < 2) {
            return res.status(400).json({ error: 'Không đủ dữ liệu kết quả (cần ít nhất 2 phiên)' });
        }

        // Khởi tạo hệ thống dự đoán
        const system = new UltraDicePredictionSystem();

        // Đưa tất cả kết quả vào hệ thống theo thứ tự từ cũ đến mới
        ...
        if (!prediction) {
            return res.status(400).json({ error: 'Không thể đưa ra dự đoán (thiếu dữ liệu hoặc model chưa đủ tin cậy)' });
        }

        // Lấy phiên hiện tại (phiên cuối cùng trong danh sách hợp lệ)
        ...
        const responseData = {
            Id: 's2king',                      // có thể lấy từ biến môi trường hoặc cố định
            Phien: phien,
            ket_qua: ketQua,
            Xuc_xac: dice,
            Phien_moi: phienMoi,
        ...
        console.error('Lỗi khi gọi API hoặc xử lý:', error.message);
...
app.listen(port, () => {
    ...
    console.log(`Truy cập /predict để lấy dự đoán`);
});