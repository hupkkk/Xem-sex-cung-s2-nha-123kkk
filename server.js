...
class UltraDicePredictionSystem {
    ...
    model2Support1() {
        ...
        return { status: "Phân tích chất lượng trend", quality };
    }

    model2Support2() {
        const reversalPoints = this.findPotentialReversals();
        return { status: "Xác định điểm đảo chiều", points: reversalPoints };
    }
    ...
    model3Support1() {
        ...
        return { status: "Phân tích hiệu quả mean reversion", effectiveness };
    }

    model3Support2() {
        const optimalThreshold = this.findOptimalDifferenceThreshold();
        return { status: "Tìm ngưỡng chênh lệch tối ưu", threshold: optimalThreshold };
    }
    ...
    model4Support1() {
        ...
        return { status: "Phân tích hiệu quả momentum ngắn hạn", effectiveness };
    }

    model4Support2() {
        const optimalTimeframe = this.findOptimalMomentumTimeframe();
        return { status: "Tối ưu khung thời gian momentum", timeframe: optimalTimeframe };
    }
    ...
    model5Support1() {
        ...
        return { status: "Phân tích đồng thuận model", consensus };
    }

    model5Support2() {
        const divergence = this.analyzeModelDivergence();
        return { status: "Phân tích phân kỳ model", divergence };
    }
    ...
    model6Support1() {
        ...
        return { status: "Phân tích hiệu quả bẻ cầu", effectiveness };
    }

    model6Support2() {
        const optimalConditions = this.findOptimalBreakConditions();
        return { status: "Xác định điều kiện bẻ cầu tối ưu", conditions: optimalConditions };
    }
    ...
    model7Support1() {
        ...
        return { status: "Phân tích phân bố trọng số", distribution: weightDistribution };
    }

    model7Support2() {
        const optimization = this.optimizeWeightAdjustment();
        return { status: "Tối ưu điều chỉnh trọng số", optimization };
    }
    ...
    model8Support1() {
        ...
        return { status: "Phân tích đặc điểm cầu xấu", characteristics };
    }

    model8Support2() {
        const strategies = this.suggestStrategiesForBadPatterns();
        return { status: "Đề xuất chiến lược cho cầu xấu", strategies };
    }
    ...
    model9Support1() {
        ...
        return { status: "Phân tích độ phức tạp pattern", complexity };
    }

    model9Support2() {
        const viability = this.assessPatternViability();
        return { status: "Đánh giá khả năng tồn tại pattern", viability };
    }
    ...
    model10Support1() {
        ...
        return { status: "Phân tích yếu tố ảnh hưởng bẻ cầu", factors };
    }

    model10Support2() {
        const forecast = this.forecastBreakProbability();
        return { status: "Dự báo xác suất bẻ cầu", forecast };
    }
    ...
    forecastBreakProbability() {
        const lastResult = this.history[this.history.length - 1] || 'T';
        const currentStreak = this.sessionStats.streaks[lastResult];
    ...
    model11Support1() {
        ...
        return { status: "Phân tích nguyên nhân biến động", causes };
    }

    model11Support2() {
        const forecast = this.forecastVolatility();
        return { status: "Dự báo biến động", forecast };
    }
    ...
    model12Support1() {
        ...
        return { status: "Phân tích hiệu suất mẫu ngắn", performance };
    }

    model12Support2() {
        const optimization = this.optimizeShortPatternLength();
        return { status: "Tối ưu độ dài mẫu ngắn", optimization };
    }
    ...
    model13Support1() {
        ...
        return { status: "Phân tích xu hướng hiệu suất", trends };
    }

    model13Support2() {
        const improvements = this.suggestPerformanceImprovements();
        return { status: "Đề xuất cải thiện hiệu suất", improvements };
    }
    ...
    model14Support1() {
        ...
        return { status: "Phân tích yếu tố bẻ cầu xu hướng", factors };
    }

    model14Support2() {
        const forecast = this.forecastTrendBreakProbability();
        return { status: "Dự báo xác suất bẻ cầu xu hướng", forecast };
    }
    ...
    model15Support1() {
        ...
        return { status: "Phân tích risk/reward theo xu hướng", analysis };
    }

    model15Support2() {
        const optimization = this.optimizeTrendDecisionThreshold();
        return { status: "Tối ưu ngưỡng quyết định xu hướng", optimization };
    }
            ...
            for (let i = 10; i < this.history.length; i++) {
                ...
                if (trend.strength > 0.6) {
                    ...
                    const prediction = shouldFollow ?
                        (trend.trend === 'up' ? 'T' : 'X') :
                        (trend.trend === 'up' ? 'X' : 'T');
                    profit += this.history[i] === prediction ? 1 : -1;
                }
            }
    ...
    model16Support1() {
        ...
        return { status: "Phân tích độ tin cậy xác suất bẻ", reliability };
    }

    model16Support2() {
        const optimization = this.optimizeBreakProbabilityWeights();
        return { status: "Tối ưu trọng số xác suất bẻ", optimization };
    }
    ...
    model17Support1() {
        ...
        return { status: "Phân tích ảnh hưởng điều chỉnh trọng số", impact };
    }

    model17Support2() {
        const optimization = this.optimizeWeightAdjustmentFrequency();
        return { status: "Tối ưu tần suất điều chỉnh trọng số", optimization };
    }
    ...
    model18Mini(data) {
        ...
        if (tCount > xCount * 1.5) {
            ...
            prediction = data[data.length - 1] === 'T' ? 'X' : 'T';
            confidence = 0.55; trend = 'Cân bằng';
        }
        return { prediction, confidence, trend };
    }
    ...
    model18Support1() {
        ...
        return { status: "Phân tích độ nhạy xu hướng ngắn hạn", sensitivity };
    }

    model18Support2() {
        const optimization = this.optimizeShortTermTrendTimeframe();
        return { status: "Tối ưu khung thời gian xu hướng ngắn hạn", optimization };
    }
    ...
    model19Support1() {
        ...
        return { status: "Phân tích sự ổn định xu hướng", stability };
    }

    model19Support2() {
        const forecast = this.forecastCommonTrends();
        return { status: "Dự báo xu hướng phổ biến", forecast };
    }
    ...
    model20Support1() {
        ...
        return { status: "Phân tích tính ổn định model hiệu suất cao", stability };
    }

    model20Support2() {
        const optimization = this.optimizeModelCombinationCount();
        return { status: "Tối ưu số lượng model trong combination", optimization };
    }
            ...
            for (let i = 20; i < this.history.length; i++) {
                ...
                for (const model of Object.keys(this.performance)) {
                    ...
                    for (let j = 10; j < i; j++) {
                        const segment = this.history.slice(j - 5, j);
                        const trend = this.model2Mini(segment);
                        const prediction = trend.trend === 'up' ? 'T' : 'X';
                        if (this.history[j] === prediction) correct++;
                        total++;
                    }
                ...
                let xCount = 0;
                for (const [model] of topModels) {
                    const trend = this.model2Mini(this.history.slice(i - 10, i));
                    const pred = trend.trend === 'up' ? 'T' : 'X';
                    if (pred === 'T') tCount++;
                    else xCount++;
                }
    ...
    model21Support1() {
        ...
        return { status: "Phân tích hiệu quả cơ chế cân bằng", effectiveness };
    }

    model21Support2() {
        const optimization = this.optimizeBalancingThreshold();
        return { status: "Tối ưu ngưỡng cân bằng", optimization };
    }
        ...
        for (let i = 20; i < this.history.length; i++) {
            ...
            for (const model of Object.keys(this.models)) {
                if (model.startsWith('model') && !model.includes('Support') && !model.includes('Mini')) {
                    simulatedPredictions[model] = this.models[model]();
                }
            }
...
async function fetchAndUpdatePrediction() {
    ...
    try {
        ...
        if (sessions.length === 0) {
            ...
            // Nếu đã có lịch sử trước đó → vẫn tính dự đoán
            if (historyResults.length > 0) {
                _buildLatestPrediction(null, null);
            }
            return;
        }
        ...
        // ----------------------------------------------------------------
        // API trả về mảng phiên sắp xếp: sessions[0] là phiên MỚI NHẤT
        // sessions[0] thường là phiên đang chạy (chưa có kết quả)
        // sessions[1] trở đi là phiên đã có kết quả
        //
        // Chiến lược:
        //   - Lấy kết quả từ tất cả phiên ĐÃ CÓ KẾT QUẢ (result = T/X)
        //     để xây dựng lịch sử; dùng id để tránh thêm trùng
        //   - Phiên hiển thị (Phien, ket_qua, Xuc_xac):
        //       • Nếu sessions[0] đã có kết quả → dùng sessions[0]
        //       • Nếu sessions[0] chưa có → dùng sessions[1] (phiên cuối đã xong)
        //   - Phien_moi: id của sessions[0] + 1 (phiên tiếp theo cần dự đoán)
        // ----------------------------------------------------------------

        // Xác định phiên đã hoàn thành gần nhất (để hiển thị kết quả thực tế)
        let displaySession = null;
        for (const s of sessions) {
            const r = getValue(s, ['result', 'ketqua', 'ket_qua', 'status', 'outcome'], '').toUpperCase();
            if (r === 'T' || r === 'X') {
                displaySession = s;
                break; // lấy phiên đầu tiên đã có kết quả
            }
        }

        // Phiên mới nhất (có thể chưa có kết quả) để lấy id cho Phien_moi
        const newestSession = sessions[0] || {};
        const newestId = getValue(newestSession, ['id', 'session_id', 'sessionId', 'phien'], 0);

        // Thêm kết quả mới vào lịch sử (chống trùng bằng id)
        let addedCount = 0;
        // Đảo ngược để thêm từ cũ → mới
        const reversed = [...sessions].reverse();
        for (const s of reversed) {
            const sid = getValue(s, ['id', 'session_id', 'sessionId', 'phien'], null);
            const r = getValue(s, ['result', 'ketqua', 'ket_qua', 'status', 'outcome'], '').toUpperCase();
            if ((r === 'T' || r === 'X') && sid !== null && !processedIds.has(sid)) {
        ...
        if (addedCount > 0) {
            console.log(`➕ Thêm ${addedCount} kết quả mới vào lịch sử`);
        }

        // Giới hạn lịch sử 200 phiên gần nhất
        if (historyResults.length > 200) {
            historyResults = historyResults.slice(-200);
        }
        console.log(`📊 Lịch sử: ${historyResults.length} kết quả`);

        // Xây dựng dự đoán và cập nhật latestPrediction
        ...
        // Nếu đang có lịch sử → vẫn cố tạo dự đoán từ lịch sử cũ
        if (historyResults.length > 0 && !latestPrediction) {
            _buildLatestPrediction(null, null);
        }
    } finally {
        isUpdating = false;
...
function _buildLatestPrediction(displaySession, newestId) {
    ...
    if (!prediction) {
        // Hoàn toàn không có dữ liệu: không đặt giá trị cứng, chỉ báo lỗi
    ...
    if (displaySession) {
        phien = getValue(displaySession, ['id', 'session_id', 'sessionId', 'phien'], 0);
        const ketQuaRaw = getValue(displaySession, ['result', 'ketqua', 'ket_qua', 'status', 'outcome'], '');
        ketQua = ketQuaRaw.toUpperCase() === 'T' ? 'Tài'
               : ketQuaRaw.toUpperCase() === 'X' ? 'Xỉu'
               : 'Chưa có';
        const rawDice = getValue(displaySession,
            ['dice', 'dices', 'xuc_xac', 'numbers', 'value', 'xucxac'], null);
        xucXac = normalizeDice(rawDice);
    }

    // Phiên mới cần dự đoán = id phiên mới nhất + 1
    // Nếu newestId là id phiên đang chạy → +1 là phiên sau đó
    // Nếu displaySession cũng là newestSession → phienMoi = phien + 1
    const baseId = (newestId !== null && newestId !== undefined) ? newestId : phien;
    ...
    console.log(`✅ Dự đoán cập nhật lúc ${lastUpdateTime.toLocaleTimeString()}:`, latestPrediction);
}
